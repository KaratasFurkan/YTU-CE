...
- *Activities* - uygulamalarda gözüken sayfalar
- *Services* - arka planda çalışan işlemler
- *Broadcast Receivers* - interruptlardır, gelen aramalar, düşük şarjda kameranın kapanması
- *Content Providers* - kalıcı verilerin kaydedilmesi, alınması, yönetilmesi sağlanır
- Intens (Activating Components) - yukarıdaki dörtlü intentler ile çalışmaya başlar
- AndroidManifest File - uygulama ile ilgili bilgiler, sistem gereksinimleri, min. os versiyonu
- App Resources - resimler, animasyonlar, tasarımlar, renkler, yazılar (strings)
- Device Configuration Changes - ekranın yan döndürülmesi, aktivitenin kapanması, fragment
- App Permissions - … permissions, runtime permissions
